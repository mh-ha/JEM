...
def plot_img(output_path, epoch, i, x):
    spath = os.path.join(output_path, 'images')
    pathlib.Path(spath).mkdir(parents=True, exist_ok=True)
    sfile = os.path.join(spath, 'epoch_{}_{}.png'.format(epoch, i))
    x_c = torch.clamp(x, -1, 1)
    save_image(x, sfile, normalize=True, nrow=sqrt(x_c.size(0)))
    wandb.log({'gen_img/epoch_{}_{}.png'.format(epoch, i): wandb.Image(sfile)})
...
def main(args):
    ...
    output_path= os.path.join(args.output_dir, 'ori_jem', args.exp)
    pathlib.Path(output_path).mkdir(parents=True, exist_ok=True)
    wandb_path= os.path.join(args.wandb_dir, 'ori_jem', args.exp)
    pathlib.Path(wandb_path).mkdir(parents=True, exist_ok=True)
    wandb.init(project="ori_jem", name='', tags='',
               dir=wandb_path, entity='ml_research')
        ...
        for i, (x_p_d, _) in tqdm(enumerate(dload_train)):
            ...
            if args.p_x_weight > 0:  # maximize log p(x)
                if args.class_cond_p_x_sample:
                    ...
                    x_q, _ = sample_q(f, replay_buffer, y=y_q)
                else:
                    # here
                    x_q, ptb_nm = sample_q(f, replay_buffer)  # sample from log-sumexp
                ...
                if cur_iter % args.print_every == 0:
                    print('P(x) | {}:{:>d} f(x_p_d)={:>14.9f} f(x_q)={:>14.9f} d={:>14.9f}, p_nm={:>14.9f}'.format(epoch, i, fp, fq, fp - fq, ptb_nm))
            ...
            if args.p_x_y_weight > 0:  # maximize log p(x, y)
                ...
                x_q_lab, _ = sample_q(f, replay_buffer, y=y_lab)
...
def get_data(args):
    ...
    _dload_train_labeled = DataLoader(dset_train_labeled, batch_size=args.batch_size, shuffle=True, num_workers=4, drop_last=True)
    dload_train_labeled = cycle(_dload_train_labeled)
...
def get_sample_q(args, device):
    ...
    def sample_q(f, replay_buffer, y=None, n_steps=args.n_steps):
        ...
        xs = init_sample.detach()
        x_k = t.autograd.Variable(init_sample, requires_grad=True)
        # print(x_k[0][0][0])
        # g = x_k.grad if x_k.grad is None else x_k.grad[0][0][0]
        # print(g)
        # import ipdb; ipdb.set_trace()
        # sgld
        for k in range(n_steps):
            f_val = f(x_k, y=y)
            f_val = f_val.sum()
            f_prime = t.autograd.grad(f_val, [x_k], retain_graph=True)[0]
            # print([p.grad for p in f.parameters()])
        ...
        with torch.no_grad():
            ptb = (xs - x_k).detach()
            ptb_norm = torch.norm(ptb.view(ptb.size(0), -1), dim=1).mean()
        return final_samples, ptb_norm
    return sample_q
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prof", action="store_true", help="")

    parser.add_argument('--wandb_dir', default=os.path.expanduser("~/wandb.log/ori_jem/"),
